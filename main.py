...
def merge(A, p, q, r):
  ...
  while index_of_right_array<len(right_part_of_array):
    ...
    index_of_right_array=index_of_right_array+1
    general_index=general_index+1
##Объяснение работы алгоритма merge sort:
# Изначальный массив разбивается на подмассивы одинакового размера, пока у нас не будет отдельных элементов 
# Эти маленькие части сравниваются между собой: 
# # Сравниваются первые элементы левого и правого массива, в том массиве, где значение оказалось меньше, указатель переносится на следующий элемент
# # Повторяется до тех пор, пока указатель одного из массивов не обойдёт его
# # Оставшийся массив дописываем справа
# например, 5 2 4 6 1 3 2 6 #
# 5 2 4 6 - 1 3 2 6
# 5 2 - 4 6 -- 1 3 2 6
# 5 - 2 -- 4 6 --- 1 3 2 6 (5 < 2) [5],[2]
# 2 5 -- 4 - 6 --- 1 3 2 6 (4 < 6) [4],[6]
# 2 5 - 4 6 -- 1 3 2 6 (2 < 4) [2,5], [4,6], l_i=0+1,r_i=0
# 2 x - x x -- 1 3 2 6 (5 > 4) [2,5], [4,6], l_i=1,r_i=0+1
# 2 4 - x x -- 1 3 2 6 (5 < 6) [2,5], [4,6], l_i=1+1,r_i=1
# 2 4 - 5 x -- 1 3 2 6 левый индекс =2, остаток правого массива дописывается в конец области
# 2 4 5 6 -- 1 3 2 6
# дальше по аналогии
